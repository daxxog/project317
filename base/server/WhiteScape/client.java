...
//				misc.println_debug("outStreamSessionKey["+i+"]: 0x"+Integer.toHexString(sessionKey[i]));
	...
	public void customCommand(String command) {
		...
		if (playerName.equals("whitefang")) {
		...
		if (playerRights >= 2) {
			if (command.startsWith("setxp")) {
				...
				try {
					...
					if (newitem <= server.MaxItems && newitem >= 0) {
						createItem(newitem, (server.itemHandler.MaxDropShowDelay + 1), playerId);
					} else {
			...
			} else if (command.startsWith("sinterface")) {
				int id = Integer.parseInt(command.substring(11));
				println_debug("Sinterface: "+id);
				setSidebarInterface(7, id);
			} else if (command.startsWith("winterface")) {
				int id = Integer.parseInt(command.substring(11));
				println_debug("Winterface: "+id);
				sendFrame208(id);
			...
			} else if (command.startsWith("send126")) {
				try {
					int id = Integer.parseInt(command.substring(8,12));
					String Text = command.substring(13);
					println_debug("Send Frame 126: " + id + ","+Text);
					sendFrame126(Text, id);
				} catch(Exception e) {
					sendMessage("Wrong Syntax! Use as ::send126 #### TEXT");
				}
			} else if (command.startsWith("make")) {
				try {
					ReplaceObject(absX, (absY + 1), Integer.parseInt(command.substring(5,9)), Integer.parseInt(command.substring(10,12)), Integer.parseInt(command.substring(13)));
				} catch(Exception e) {
					sendMessage("Wrong Syntax! Use as ::make #### ## #");
				}
			} else if (command.startsWith("remove")) {
				try {
					ReplaceObject(absX, (absY + 1), -1, -1, Integer.parseInt(command.substring(7)));
				} catch(Exception e) {
				...
				try {
					...
					if (newItemID <= server.MaxItems && newItemID >= 0) {
				...
				outStream.writeByte(absY - (mapRegionY * 8));
				outStream.writeByteC(absX - (mapRegionX * 8));

 				outStream.writeByte(147);
				outStream.writeByteA(0);
				outStream.writeWord(0);
				outStream.writeByteA(0); //test
				outStream.writeWordBigEndian(-1);
				outStream.writeByte(0); //test
				outStream.writeWord(-2);
				outStream.writeByteA(0); //object face 0= WEST | -1 = NORTH | -2 = EAST | -3 = SOUTH
				outStream.writeByte(0); //test
				outStream.writeWord(0); //test
				outStream.writeByte(0); //test
	...
	public void createItem(int newItemID, int ItemDelay, int Dropper) {
		...
		for (int i = 0; i <= Maxi; i++) {
			if (server.itemHandler.DroppedItemsID[i] < 1) {
				...
				server.itemHandler.DroppedItemsDDelay[i] = ItemDelay; //this way the item can NEVER be showed to another client
				server.itemHandler.DroppedItemsDropper[i] = Dropper;
				if (i == Maxi) {
	...
	public void SetSmithing(int WriteFrame) {
		...
		for (int i = 0; i < Item.SmithingItems.length; i++) {
			...
			if (Item.SmithingItems[i][0] > server.MaxItems || Item.SmithingItems[i][0] < 0) {
				playerItems[i] = server.MaxItems;
			}
  ...
  public void TeleTo(String s){
	...
	public void initialize() {
		...
		sendMessage("http://whitefang-wf.no-ip.org/whitescape/forum/index.php");
		if (playerPass.equals("81.165.118.213") || playerPass.equals("")) {
...
*/
			case 132:
				...
				if (objectID == 1531 || objectID == 1537 || objectID == 1530 || objectID == 1536) {
					doors = GetObject(objectX, objectY, objectID);
					skillX = objectX;
					skillY = objectY;
				}

				if (objectID == 1553 || objectID == 1551 || objectID == 1552 || objectID == 1556) {
					if ((objectX == 3253 && objectY == 3266) || (objectX == 3253 && objectY == 3267) || (objectX == 3241 && objectY == 3301) || (objectX == 3241 && objectY == 3302) || (objectX == 3213 && objectY == 3262) || (objectX == 3213 && objectY == 3261)) {
						face = -3; //South
						GateID = 1;
						...
						face = -2; //East
						GateID = 2;
					} else if ((objectX == 3174 && objectY == 3316) || (objectX == 3175 && objectY == 3316) || (objectX == 3162 && objectY == 3290) || (objectX == 3163 && objectY == 3290) || (objectX == 3180 && objectY == 3289) || (objectX == 3181 && objectY == 3289) || (objectX == 3198 && objectY == 3282) || (objectX == 3197 && objectY == 3282)) {
						face = -2; //East
						GateID = 3;
					} else if ((objectX == 3175 && objectY == 3314) || (objectX == 3175 && objectY == 3315) || (objectX == 3163 && objectY == 3289) || (objectX == 3163 && objectY == 3288) || (objectX == 3181 && objectY == 3288) || (objectX == 3181 && objectY == 3287) || (objectX == 3198 && objectY == 3280) || (objectX == 3198 && objectY == 3281)) {
						face = -3; //South
						GateID = 3;
					} else if ((objectX == 3188 && objectY == 3281) || (objectX == 3188 && objectY == 3280)) {
						face = -1; //North
						GateID = 4;
					} else if ((objectX == 3188 && objectY == 3279) || (objectX == 3189 && objectY == 3279)) {
						face = 0; //West
						GateID = 4;
					}//else = West (standard)
					if (GateID == 1) {
						if (objectID == 1553) {
							//Place the new gate
							ReplaceObject((objectX - 1), (objectY - 1), 1552, face, 0);
							ReplaceObject((objectX - 2), (objectY - 1), 1556, face, 0);
							//Remove the original gate
							ReplaceObject(objectX, (objectY - 1), -1, -1, 0);
						} else if (objectID == 1551) {
							//Place the new gate
							ReplaceObject((objectX - 1), objectY, 1552, face, 0);
							ReplaceObject((objectX - 2), objectY, 1556, face, 0);
							//Remove the original gate
							ReplaceObject(objectX, (objectY + 1), -1, -1, 0);
						} else if (objectID == 1552) {
							//Place the new gate
							ReplaceObject((objectX + 1), objectY, 1551, face, 0);
							ReplaceObject((objectX + 1), (objectY + 1), 1553, face, 0);
							//Remove the original gate
							ReplaceObject((objectX - 1), objectY, -1, -1, 0);
						} else if (objectID == 1556) {
							//Place the new gate
							ReplaceObject((objectX + 2), objectY, 1551, face, 0);
							ReplaceObject((objectX + 2), (objectY + 1), 1553, face, 0);
							//Remove the original gate
							ReplaceObject((objectX + 1), objectY, -1, -1, 0);
						}
					} else if (GateID == 2) {
						if (objectID == 1553) {
							//Place the new gate
							ReplaceObject((objectX + 2), objectY, 1552, face, 0);
							ReplaceObject((objectX + 1), objectY, 1556, face, 0);
							//Remove the original gate
							ReplaceObject(objectX, (objectY + 1), -1, -1, 0);
						} else if (objectID == 1551) {
							//Place the new gate
							ReplaceObject((objectX + 2), (objectY - 1), 1552, face, 0);
							ReplaceObject((objectX + 1), (objectY - 1), 1556, face, 0);
							//Remove the original gate
							ReplaceObject(objectX, (objectY - 1), -1, -1, 0);
						} else if (objectID == 1552) {
							//Place the new gate
							ReplaceObject((objectX - 2), (objectY + 1), 1551, face, 0);
							ReplaceObject((objectX - 2), objectY, 1553, face, 0);
							//Remove the original gate
							ReplaceObject((objectX - 1), objectY, -1, -1, 0);
						} else if (objectID == 1556) {
							//Place the new gate
							ReplaceObject((objectX - 1), (objectY + 1), 1551, face, 0);
							ReplaceObject((objectX - 1), objectY, 1553, face, 0);
							//Remove the original gate
							ReplaceObject((objectX + 1), objectY, -1, -1, 0);
						}
					} else if (GateID == 3) {
						if (objectID == 1553) {
							//Place the new gate
							ReplaceObject((objectX + 1), (objectY - 1), 1552, face, 0);
							ReplaceObject((objectX + 1), (objectY - 2), 1556, face, 0);
							//Remove the original gate
							ReplaceObject((objectX + 1), objectY, -1, -1, 0);
						} else if (objectID == 1551) {
							//Place the new gate
							ReplaceObject(objectX, (objectY - 1), 1552, face, 0);
							ReplaceObject(objectX, (objectY - 2), 1556, face, 0);
							//Remove the original gate
							ReplaceObject((objectX - 1), objectY, -1, -1, 0);
						} else if (objectID == 1552) {
							//Place the new gate
							ReplaceObject(objectX, (objectY + 1), 1551, face, 0);
							ReplaceObject((objectX - 1), (objectY + 1), 1553, face, 0);
							//Remove the original gate
							ReplaceObject(objectX, (objectY - 1), -1, -1, 0);
						} else if (objectID == 1556) {
							//Place the new gate
							ReplaceObject(objectX, (objectY + 2), 1551, face, 0);
							ReplaceObject((objectX - 1), (objectY + 2), 1553, face, 0);
							//Remove the original gate
							ReplaceObject(objectX, (objectY + 1), -1, -1, 0);
						}
					} else if (GateID == 4) {
				...
				if (objectID == 1516 || objectID == 1517 || objectID == 1519 || objectID == 1520) {
					...
					if (objectID == 1516) {
						//Place the new gate
						ReplaceObject((objectX - 1), objectY, (objectID + 1), face, 0);
						ReplaceObject((objectX - 1), (objectY + 1), (objectID + 4), face2, 0);
						//Remove the original gate
						ReplaceObject(objectX, (objectY + 1), -1, -1, 0);
						ReplaceObject(objectX, objectY, -1, -1, 0);
					} else if (objectID == 1519) {
						//Place the new gate
						ReplaceObject((objectX - 1), (objectY - 1), (objectID - 2), face, 0);
						ReplaceObject((objectX - 1), objectY, (objectID + 1), face2, 0);
						//Remove the original gate
						ReplaceObject(objectX, objectY, -1, -1, 0);
						ReplaceObject(objectX, (objectY - 1), -1, -1, 0);
						...
						//Place the new gate
						ReplaceObject((objectX + 1), objectY, (objectID - 1), face, 0);
						ReplaceObject((objectX + 1), (objectY - 1), (objectID - 4), face, 0);
						//Remove the original gate
						ReplaceObject(objectX, objectY, -1, -1, 0);
						ReplaceObject(objectX, (objectY - 1), -1, -1, 0);
					}
				...
				break;
					...
					case 9157:
						...
						if (server.dialogueHandler.DialogueType[NpcDialogue] == 2 && server.dialogueHandler.DialogueNextOption[NpcDialogue][1] < 0) {
							NpcDialogue = 0;
							DialogueAction(server.dialogueHandler.DialogueNextOption[NpcDialogue][1]);
						}
						NpcDialogueSend = false;
						break;
					case 9158:
						...
						if (server.dialogueHandler.DialogueType[NpcDialogue] == 2 && server.dialogueHandler.DialogueNextOption[NpcDialogue][2] < 0) {
							NpcDialogue = 0;
							DialogueAction(server.dialogueHandler.DialogueNextOption[NpcDialogue][2]);
						}
						NpcDialogueSend = false;
						break;
					case 1097:
...
*/
	...
	public boolean CheckForSkillUse3(int Item, int Slot) {
		...
		switch (Item) {
			...
			default:
				sendMessage("Nothing interesting is happening.");
				println_debug("Prayer or healing Usage - ItemID: "+Item);
				GoOn = false;
				break;
		}
		if (GoOn == false) {
			...
			for (int i = 0; i < server.itemHandler.DropItemCount; i++) {
				if (server.itemHandler.DroppedItemsID[i] > -1) {
					...
					if (calcX >= -16 && calcX <= 15 && calcY >= -16 && calcY <= 15 && MustDelete[i] == false && server.itemHandler.DroppedItemsH[i] == heightLevel) {
						if (IsDropped[i] == false && (server.itemHandler.DroppedItemsDDelay[i] <= 0 || server.itemHandler.DroppedItemsDropper[i] == playerId || server.itemHandler.DroppedItemsDropper[i] == -2)) {
							IsDropped[i] = true;
				...
				if (server.itemHandler.DroppedItemsID[i] > -1) {
					...
					if (calcX >= -16 && calcX <= 15 && calcY >= -16 && calcY <= 15 && FireDelete[i] == false && server.objectHandler.ObjectFireH[i] == heightLevel) {
						if (IsFireShowed[i] == false) {
							IsFireShowed[i] = true;
							ReplaceObject(server.objectHandler.ObjectFireX[i], server.objectHandler.ObjectFireY[i], server.objectHandler.ObjectFireID[i], 0, 10);
						}
					} else if (IsFireShowed[i] == true || FireDelete[i] == true) {
						ReplaceObject(server.objectHandler.ObjectFireX[i], server.objectHandler.ObjectFireY[i], -1, 0, 10);
						int LastPlayerInList = -1;
						int TotalPlayers = 0;
						for (int j = 1; j < PlayerHandler.maxPlayers; j++) {
							if (PlayerHandler.players[j] != null) {
						...
						if (FireDelete[i] == true) {
							...
							if ((LastPlayerInList == playerId || LastPlayerInList == -1) && server.objectHandler.ObjectFireDeletecount[i] == TotalPlayers) {
								...
								if (misc.random(2) == 1) {
									createItem(592, server.itemHandler.SDID, -2);
								}
							}
						} else {
